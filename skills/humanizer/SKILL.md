...
If the user gives a writing sample, read `references/voice-fingerprint.md` and build a voice fingerprint before rewriting. Match recurring sentence or line length, syntax, word choice, punctuation, openings, transitions, tone, detail density, and deliberate quirks. Explicit user preferences outrank inferred traits. The sample overrides the generic patterns below, including §8: if the sample naturally uses dashes, keep them at about the same rate. Do not claim permanent learning; export a reusable voice profile only when the user asks.
...
For song lyrics, rap verses, hooks, or lyric critique, read `references/lyrics.md` before working. Before a lyric rewrite, also read `references/artifact-scoring.md` and score the source lines for artifact risk so the edit targets the actual problem instead of replacing strong lines indiscriminately. In pasted lyric mode, show the compact score report unless the user asks for only the finished lyrics. Re-score the rewrite internally before returning it.

For rap, melodic rap, drill, trap, boom-bap, freestyle-style writing, or any request involving bars, flow, pocket, cadence, rhyme density, internal rhyme, multisyllabic rhyme, rhyme schemes, or punchlines, also read `references/rap-technique.md`. If the user asks to score or grade rap technique, also read `references/rap-scoring.md` and keep multisyllabic rhyme, pocket, cadence, and rhyme density as separate 0–5 scores. Do not average them into one quality number unless explicitly asked. Stress placement and speakable pocket outrank raw syllable count. Meaning outranks technical density. Preserve concrete user-authored details and never invent personal facts to make a verse sound authentic. When lyric samples are supplied, combine lyric mode, artifact scoring, rap technique, rap scoring when requested, and the voice fingerprint. If the user asks to improve weak rap-technique scores while preserving strong lines, read `references/score-guided-rewrite.md` and use its freeze/target/acceptance rules instead of rewriting the verse wholesale. In score-guided rewrite mode, generate three meaning-preserving candidates for each editable weak bar, rank them with the tournament rules in that reference, and keep only the strongest acceptable candidate. Unless the user asks to see all candidates, show the original and selected rewrite side by side with exact before→after score deltas rather than exposing discarded drafts. If the user asks to optimize a full 16-bar verse, strengthen the verse as a whole, or consider how each bar affects the bars before and after it, also read `references/verse-optimizer.md`. Use its dependency map and local/block/whole-verse acceptance gates so isolated score gains cannot break setup/payoff, rhyme chains, cadence motifs, breath sequence, callbacks, or narrative progression. If the user asks to optimize verses and hooks together, improve how a verse leads into a chorus, compare section roles, or strengthen whole-song cohesion, also read `references/song-section-optimizer.md` and apply its hook-contract, transition, and cross-section acceptance rules. If the user supplies BPM and time signature, asks for beat-aware pocket/cadence analysis, or wants grid-aware scoring, also read `references/beat-aware-pocket.md`; treat BPM/meter as a better timing constraint, not proof of exact performed microtiming when no performance audio or timestamps are available. If the user asks for the best flow pattern for each bar, bar-by-bar flow suggestions, rhythmic delivery architecture, or flow choices based on BPM/meter plus the surrounding lyrics, also read `references/flow-pattern-generator.md`. Generate continuity, variation, and contrast candidates internally, reject patterns that fight natural stress or context, and select the strongest pattern across the bar, local context, and section rather than forcing every line into one cadence.
